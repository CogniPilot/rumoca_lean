...
sed 's/valid = 0/valid = 1/' build/tensor-c/finite.c > build/tensor-c/corrupt-finite.c
cat > build/tensor-c/Reject-finite.lean <<'LEAN'
import RumocaC.TensorArtifactCheck
verify_tensor_helper "build/tensor-c/corrupt-finite.c" as finite
LEAN
if lake env lean build/tensor-c/Reject-finite.lean > build/tensor-c/finite-rejection.log 2>&1; then
  echo 'corrupted tensor finite scanner passed its actual-file contract' >&2
  exit 1
fi
