...
def certificate (sourceName source actualC : String) (m : AST.Model) : String :=
  ...
  "theorem compilation_checked : compile input = .ok artifact := by\n" ++
  "  exact compile_eq_parsed input parsed resolved\n\n" ++
  ...
  "theorem source_to_c : compile input = .ok artifact ∧ ArtifactContract artifact emitted :=\n" ++
...
def main (args : List String) : IO UInt32 := do
  ...
  | [sourcePath, cPath, proofPath] =>
    try
      ...
      match compile (.single sourcePath source) with
      ...
      | .ok a =>
        ...
        IO.FS.writeFile proofPath (certificate sourcePath source actualC a.parsed.ast)
        return (0 : UInt32)
