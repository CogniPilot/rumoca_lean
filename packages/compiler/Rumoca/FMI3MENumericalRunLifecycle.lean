...
importer chooses an admitted initialized numerical/reset history. The initial caller
...
The selected source IVP is recorded at every actual initialization checkpoint,
separately from trial states subsequently supplied by the importer. -/
theorem runtime_create_me_run_release (compiled : compile input = .ok a)
      ...
      ∀ (E : Type) (header : CFenv.Header) (instances flags : Nat) (separate : instances ≠ flags)
        ...
        ∃ live, ∃ initial : Binary64.Value,
          ...
          (∀ (initArgs : Initialization.Arguments) (addresses : String → Address) (buffer : Address)
            (actions : List MENumericalRun.Action) (final : MENumericalHistory.ReferenceState),
            initArgs.Admissible → MENumericalHistory.CallerStorage heap objects.instances addresses buffer →
            MENumericalRun.ReferenceTrace (MENumericalHistory.ReferenceState.initial initArgs initial) actions final →
            ...
            ∃ after finalClock epochs,
              MENumericalRun.Calls program p addresses buffer exited actions
                (MENumericalRun.observations a.solve.prepareFMI3
                  (MENumericalHistory.ReferenceState.initial initArgs initial) actions) after epochs ∧
              MENumericalHistory.Stored after p finalClock final addresses buffer ∧
              Reset.Storage after p ∧ MENumericalRun.InitializedEpochs a.parsed.ast p epochs ∧
              CReadOnly.Preserves heap after ∧
              (∀ observed actualAfter actualEpochs,
                MENumericalRun.Executed program p addresses buffer exited actions observed actualAfter actualEpochs →
                observed = (MENumericalRun.observations a.solve.prepareFMI3
                ...
                MENumericalRun.DerivativeObservations a.parsed.ast actions observed ∧
                actualEpochs = epochs ∧ MENumericalRun.InitializedEpochs a.parsed.ast p actualEpochs) ∧
  ...
  obtain ⟨entered, exited, after, finalClock, epochs, called, finalStored, finalReset, readonly,
    terminated, discharged, ownersAfter, freed, frame⟩ :=
    MENumericalRun.initialize_release header objects literals a.solve.prepareFMI3 program tag
      enterDefined exitDefined (prepared.quiet header E objects firstBlock program actual) reset finish
  ...
  refine ⟨entered, exited, initialized,
    ...
    after, finalClock, epochs, called, finalStored, finalReset, called.epochs_source a.solve,
    (CCalls.Events.termination_preserves ((creation _).mpr rfl)).trans readonly,
    ?_, terminated, discharged, ownersAfter, freed, ?_⟩
  · intro observed actualAfter actualEpochs executed
    obtain ⟨values, finalHeap, checkpoints⟩ := called.determines executed
    refine ⟨values, finalHeap, ?_, checkpoints, ?_⟩
    · rw [values]
      exact MENumericalRun.observations_source a.solve (MENumericalHistory.ReferenceState.initial initArgs initial) actions
    · rw [checkpoints]
      exact called.epochs_source a.solve
