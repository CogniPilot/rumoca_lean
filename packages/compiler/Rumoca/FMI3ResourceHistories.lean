...
/-- The checked source/artifact program supplies the same release contract
and coupled resource/publication history. Starting from an empty host ledger,
all later links and API origins are derived; usable handles have actual factory
observations. Factory retention, buffers and callback frames remain explicit
protocol boundaries, and native correspondence is not claimed. -/
theorem source_resource_histories (compiled : compile input = .ok a)
  ...
  obtain ⟨compiled, numerical, metadataContract, sigs, pool, prepared, rendered, functions, covered, implementation⟩ :=
    source_authorized_release compiled contract
  ...
  obtain ⟨program, actual, callback, statics, floor, rounding, release⟩ :=
  ...
  intro policy
  refine ⟨release policy, ?_⟩
  intro heap ticks after path
  obtain ⟨invariant, resources, publications⟩ := Resources.Publication.initial_history path
  exact ⟨invariant, resources, publications, fun handle owned =>
    Resources.Publication.initial_handle_origin path owned⟩
