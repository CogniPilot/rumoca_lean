...
theorem SourcePrefix.of_compiler [CInterface] {source : AST.Model} {model : Solve.FMI3Model source}
    ...
    (compiler : ∀ heap actions state,
      Invariant program objects retained owners original literals heap p kind before readers →
      ReferenceTrace kind before actions state →
      (∀ action ∈ actions, action.Prepared objects retained original p access readers) →
      SourceContract model program objects retained owners original literals heap p access kind before state actions readers)
    (invariant : Invariant program objects retained owners original literals heap p kind before readers)
  ...
  have requests : ∀ action ∈ stop.done, action.Prepared objects retained original p access readers := by
    intro action member
