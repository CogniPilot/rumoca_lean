...
theorem runtime_create_release (compiled : compile input = .ok a)
  ...
  obtain ⟨sigs, unique, resetMember, printed, _, functions, _, _, queries, ready,
    _, _, _, nominals, states, derivative, getter, setter, initialization, _, factories, runtime, termination, time, entries, completed, discrete, step, logging, eventContract, evaluationContract, absentContract, _⟩ := build.adapter
