...
structure RestartSourceContract [CInterface] (model : Solve.FMI3Model source) (program : Program Invocation)
    ...
    (kind : Kind) (final : State) (actions : List Action) (readers : ReadBank) : Prop where
  ...
  initialized : SourceContract model program objects retained owners original literals (Reset.finalHeap heap p)
    p buffers kind State.reset final actions readers
...
theorem RestartSourceContract.progress [CInterface] {program : Program Invocation}
    (certified : RestartSourceContract model program objects retained owners original literals heap p buffers kind final actions readers) :
...
theorem RestartSourceContract.completed [CInterface] {program : Program Invocation}
  ...
  | cons called following =>
    ...
    refine ⟨⟨_, rfl, observations⟩, exits, invariant, certified.readonly.trans readonly,
      (by simpa using (Retention.of_retains certified.retains).trans retains), ?_⟩
...
theorem restart_source [CInterface] {program : Program Invocation}
    ...
    (compileProtocol : ∀ next, Invariant program objects retained owners original literals next p kind State.reset readers →
      SourceContract model program objects retained owners original literals next p buffers kind State.reset final actions readers) :
    RestartSourceContract model program objects retained owners original literals heap p buffers kind final actions readers := by
  obtain ⟨called, invariant, readonly, retains⟩ := reset_invariant model reset storage kindValue modeValue
    represented caller literalFrame logging readerFrame readerOutside
    ...
    ∃ sigs, ∃ pool : Pool (LiteralPreparation.excluded ++
      ...
      ∀ (header : CFenv.Header) (objects : Objects) (baseHeap : Heap) (firstBlock : Nat) (signed : Bool),
        ...
        ∀ (retained : Address → Prop) (owners : SlotOwners.State objects.capacity)
          ...
          (final : State) (actions : List Action) (readers : ReadBank),
          Resources objects retained original p buffers readers → Reset.Storage heap p →
          ...
          RestartSourceContract a.solve.prepareFMI3 program objects retained owners original
            (pool.install baseHeap firstBlock signed) heap p buffers kind final actions readers := by
  obtain ⟨compiled, numerical, metadataVariables, writable, counts, nominals, loggingMetadata, equation, sigs, pool, made, printed, functions, lifecycle, certify⟩ :=
    runtime_source compiled build
  refine ⟨compiled, numerical, metadataVariables, writable, counts, nominals, loggingMetadata, equation, sigs, pool, made, printed, functions, ?_⟩
  intro header objects baseHeap firstBlock signed
  letI : CInterface := RuntimeEnvironment.interface header objects (pool.addresses firstBlock)
  intro program actual compare retained owners original heap p buffers kind mode final actions readers resources storage
    kindValue modeValue represented caller literalFrame logging readerFrame reference prepared
  obtain ⟨reset, _, _, _, _⟩ := lifecycle.execution header objects (pool.addresses firstBlock) program actual
  exact restart_source a.solve.prepareFMI3 reset storage kindValue modeValue represented caller literalFrame logging readerFrame
    (fun q inside => (resources.readerGuarded q inside).2.1)
    (fun next invariant => certify header objects baseHeap firstBlock signed program actual compare retained owners original next p buffers
      kind State.reset final actions readers resources invariant reference prepared)
