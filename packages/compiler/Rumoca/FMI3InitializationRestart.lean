...
theorem runtime_restart_source (compiled : compile input = .ok a)
  ...
  obtain ⟨compiled, numerical, metadataVariables, writable, counts, nominals, equation, sigs, pool, made, printed, functions, lifecycle, certify⟩ :=
    runtime_source compiled build
  refine ⟨compiled, numerical, metadataVariables, writable, counts, nominals, equation, sigs, pool, made, printed, functions, ?_⟩
