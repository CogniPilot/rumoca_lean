...
structure SourceContract [CInterface] (model : Solve.FMI3Model source) (program : Program Invocation)
    ...
    (kind : Kind) (state final : State) (actions : List Action) : Prop where
...
theorem source_contract [CInterface] {program : Program Invocation}
    ...
    SourceContract model program objects retained owners original literals heap p buffers kind state final actions := by
...
theorem runtime_source (compiled : compile input = .ok a)
    ...
    ∃ sigs, ∃ pool : Pool (LiteralPreparation.excluded ++
      ...
      ∀ (header : CFenv.Header) (objects : Objects) (baseHeap : Heap) (firstBlock : Nat) (signed : Bool),
        ...
        ∀ (retained : Address → Prop) (owners : SlotOwners.State objects.capacity)
          ...
          (state final : State) (actions : List Action),
          Resources objects retained original p buffers →
          Invariant program objects retained owners original (pool.install baseHeap firstBlock signed) heap p kind state →
  ...
  exact source_contract (execution_contract header objects a.solve.prepareFMI3 sigs pool getPrepared setPrepared countPrepared nominalPrepared lifecycle
