...
theorem runtime_source (compiled : compile input = .ok a)
  ...
  obtain ⟨sigs, unique, resetMember, printed, _, functions, _, _, queries, ready,
    _, _, _, nominals, _, _, getter, setter, initialization, _, _, runtime, termination, _, _, _, _, _, logging, eventContract, evaluationContract, absentContract⟩ := build.adapter
