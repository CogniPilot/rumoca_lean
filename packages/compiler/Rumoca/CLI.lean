...
source-build certificate that `TensorFMU.build` runs, for tensor eFMI Algorithm
Code (`.alg`) output, whose publication gate is the fixed `tensor-algorithm`
certificate, and for complete tensor eFMU (`.efmu`) output, whose publication
gate is the fixed `tensor-efmi-archive` certificate that `writeTensorArchive`
runs. Tensor C emission on stdout is not built and is rejected with a
diagnostic. -/
private def runTensorCompiler {input : Source.InputRef} (name : String)
  ...
  | some path =>
    if path.endsWith ".fmu" then
      ...
      EFMIExport.writeTensorArchive tensor path
      return 0
    else
