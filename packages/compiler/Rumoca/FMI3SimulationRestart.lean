...
theorem restart_after_me [CInterface] {program : Program Invocation}
    ...
    (compileProtocol : ∀ next, Invariant program objects retained owners original literals next p .me State.reset →
      SourceContract model program objects retained owners original literals next p access .me State.reset nextState actions) :
    RestartSourceContract model program objects retained owners original literals after p access .me nextState actions := by
  ...
  exact restart_source model reset resetStorage stored.control.kind stored.mode_loaded represented
    (caller.trans (certified.storage executed policy)) (literalFrame.trans readonly) loggingAfter compileProtocol
...
theorem restart_after_cs_logged [CInterface] {program : Program Invocation}
    ...
    (compileProtocol : ∀ next, Invariant program objects retained owners original literals next p .cs State.reset →
      SourceContract model program objects retained owners original literals next p access .cs State.reset nextState actions) :
    RestartSourceContract model program objects retained owners original literals after p access .cs nextState actions := by
  ...
  exact restart_source model reset stored.reset stored.kind stored.mode represented
    ...
    (logging.framed (fun name outside => keeps name outside)) compileProtocol
...
theorem restart_after_cs_suppressed [CInterface] {program : Program Invocation}
  ...
  exact restart_source model reset stored.reset stored.kind stored.mode
    ...
    (logging.framed (fun name outside => certified.retains name outside)) compileProtocol
