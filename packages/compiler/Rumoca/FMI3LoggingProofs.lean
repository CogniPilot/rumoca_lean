...
Every represented host outcome is covered, including missing outcomes in the
atomic external machine; symbolic callback bindings remain explicit.
Native ABI and whole public APIs remain open. -/
...
theorem logging_source (compiled : compile input = .ok a)
  ...
  exact ⟨compiled, Logging.metadata_correct _ contract.metadata,
    ...
    made, logging.prepared pool made, logging.all_prepared pool made,
    logging.silent_prepared pool made, events pool made⟩
