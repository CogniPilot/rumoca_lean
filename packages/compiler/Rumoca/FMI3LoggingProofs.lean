...
theorem logging_source (compiled : compile input = .ok a)
    ...
    ∃ sigs, ∃ pool : Pool (LiteralPreparation.excluded ++
      ...
      Logging.PreparedContract a.solve.prepareFMI3 sigs pool := by
  obtain ⟨sigs, _, _, printed, _, grammar, _, _, _, ready, _, logging⟩ := contract.adapter
  ...
  exact ⟨compiled, Logging.metadata_correct _ contract.metadata,
    ...
    made, logging.prepared pool made⟩
