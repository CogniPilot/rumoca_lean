...
coordinates. The initial vocabulary is intentionally only fill and return.
Arithmetic, stencils and contractions require their own subsequent contracts.
-/
...
def Program.eval (zero one : α) : Program Γ s → Env α Γ → Value α s
  | .ret r, env => env r
  | .fill shape value next, env =>
      next.eval zero one (env.push (Value.fill shape (value.eval zero one)))
...
def Program.denote (zero one : α) : Program Γ s → SpecEnv α Γ → Denotation α s
  | .ret r, env => env r
  | .fill _ value next, env => next.denote zero one (env.push (fun _ => value.eval zero one))
...
theorem Program.eval_correct (p : Program Γ s) (zero one : α) (env : Env α Γ) :
    ∀ i, (p.eval zero one env)[i] = p.denote zero one (fun r i => (env r)[i]) i := by
  induction p with
  | ret => intro i; rfl
