...
theorem message_collected (model : Solve.FMI3Model source) (reason : Failure) :
  ...
  cases reason <;>
    simp [functionTexts, Runtime.function, body, Runtime.require, Runtime.instancePrefix,
      Runtime.modeGuard, Runtime.reject, Runtime.branch, Runtime.fail, Runtime.ret, tail,
      Runtime.call, Runtime.put, Runtime.ok, ErrorCalls.rejectionMessage,
      failureMessage, statementTexts, expressionTexts]
...
theorem prepared_correct (model : Solve.FMI3Model source) (sigs : List Signature)
  ...
  · intro before firstBlock signed objects heap frame
    ...
    have allMessages : ∀ reason, ∃ address, pool.addresses firstBlock (failureMessage reason) = some address := by
      intro reason
      exact LiteralPreparation.message_bound model sigs made signature member
        (failureMessage reason) (message_collected model reason) firstBlock
    let messages : Failure → Address := fun reason => Classical.choose (allMessages reason)
    have messageBound : ∀ reason, pool.addresses firstBlock (failureMessage reason) = some (messages reason) :=
