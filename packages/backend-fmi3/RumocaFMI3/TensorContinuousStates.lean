...
import RumocaFMI3.TensorDerivativeSuccess
import RumocaFMI3.TensorDerivativeDiscard
...
every caller value and then copies them into `x`; the derivative getter first
checks the encoded square results without heap writes. Overflow returns Discard
with the configured logging behavior; the heap remains unchanged up to any
external callback. Finite results run the prepared derivative entry (through
...
theorem getBody_closed (shape : Tensor.Shape) :
  ...
  simp [getFunction, getBody, getTail, countReject, getLoopSuffix, getCopyBody, Runtime.require,
    ...
    Runtime.ok, Runtime.v, CBodyEmbedding.closedBlocks, CLoops.noDeclarations, CLoops.loop,
    CLoops.counterStep]
  ...
  have s_src : CLoops.next (.running (getTail shape) (guardEnv p buffer count) types0 heap) =
    ...
    TensorFloat64.declare_step_e (guardEnv p buffer count) types0 heap "fmi3Float64 *" "src"
      ...
      (by apply CBodyEmbedding.eval_refines
          simp [Runtime.region, Runtime.field, Runtime.v, Runtime.n, CBody.eval, CBody.evalWith, CDeclaredMembers.memberValue, CDeclaredMembers.arrayAt, CDeclaredMembers.fieldAt, CBody.lvalueWith, guardEnv, parameters, CBody.bind,
            CBody.resolve, Value.address]) rfl
...
theorem setBody_closed (shape : Tensor.Shape) :
  ...
  simp [setFunction, setBody, setTail, countReject, setLoopSuffix, setCopyBody, validateBody,
    ...
    Runtime.fail, Runtime.ret, Runtime.ok, Runtime.v, CBodyEmbedding.closedBlocks,
...
theorem derivBody_closed (shape : Tensor.Shape) (hasOutput : Bool) :
    (derivFunction shape hasOutput).body.all CBodyEmbedding.closedBlocks = true :=
  TensorDerivativeAdmission.body_closed shape hasOutput
...
theorem deriv_enter (shape : Tensor.Shape) (p buffer : Address) (count : UInt64)
  ...
  simp [CCalls.Events.internalNext, CCalls.Events.internalNextWith, CCalls.Typed.nextWithExpressions, CLoops.nextWith, CLoops.evalWith, CBody.legacyExpressions,
    derivEntryArgs, Runtime.call, Runtime.region, Runtime.field, Runtime.v, Runtime.n, CBody.eval, CBody.evalWith, CDeclaredMembers.memberValue, CDeclaredMembers.arrayAt, CDeclaredMembers.fieldAt,
    CBody.lvalueWith,
    ...
    CCalls.argumentsWith, CBody.legacyExpressions, derivGuardEnv, derivParameters, CBody.bind, CBody.resolve, CBody.constants,
    Value.address]
...
theorem jac_enter (shape : Tensor.Shape) (p buffer : Address) (count : UInt64)
    ...
    CCalls.Events.internalNext program
      ...
      some (.calling "rumoca_square_jacobian_diag"
        ...
        (.caller .discard rest (derivGuardEnv p buffer count) types0 "fmi3Status" stack)) := by
  simp [jacobianCall, CCalls.Events.internalNext, CCalls.Events.internalNextWith, CCalls.Typed.nextWithExpressions, CLoops.nextWith, CLoops.evalWith, CBody.legacyExpressions,
    jacobianEntryArgs, Runtime.call, Runtime.region, Runtime.field, Runtime.v, Runtime.n, CBody.eval, CBody.evalWith, CDeclaredMembers.memberValue, CDeclaredMembers.arrayAt, CDeclaredMembers.fieldAt,
    CBody.lvalueWith,
    CCalls.Events.enterCallWith, CCalls.Events.resolveWith, CCalls.Indirect.operand, CCalls.Indirect.resolveWith, CBody.legacyExpressions,
    CCalls.argumentsWith, CBody.legacyExpressions, derivGuardEnv, derivParameters, CBody.bind, CBody.resolve, CBody.constants,
    Value.address]
...
theorem deriv_delivers (shape : Tensor.Shape) (H : Heap) (p buffer : Address) (count : UInt64)
  ...
  have s_src : CLoops.next (.running (derivCopyTail shape) (derivGuardEnv p buffer count) types0 H) =
    ...
    TensorFloat64.declare_step_e (derivGuardEnv p buffer count) types0 H "fmi3Float64 *" "src"
      ...
      rfl (by apply CBodyEmbedding.eval_refines
              simp [Runtime.region, Runtime.field, Runtime.v, Runtime.n, CBody.eval, CBody.evalWith, CDeclaredMembers.memberValue, CDeclaredMembers.arrayAt, CDeclaredMembers.fieldAt, CBody.lvalueWith, derivGuardEnv, derivParameters,
                CBody.bind, CBody.resolve, Value.address]) rfl
  have s_values : CLoops.next (.running (.declare "fmi3Float64 *" "values" (Runtime.v "derivatives") ::
    ...
    TensorFloat64.declare_step_e _ _ H "fmi3Float64 *" "values" (Runtime.v "derivatives") .pointer
      ...
      rfl (by simp [Runtime.v, CLoops.eval, CLoops.evalWith, CBody.legacyExpressions, CBody.eval, CBody.evalWith, derivGuardEnv, derivParameters, CBody.bind, CBody.resolve]) rfl
...
theorem deriv_reaches (shape oshape : Tensor.Shape) (definitions : CLoops.Calls.Definitions)
  ...
  exact TensorDerivativeSuccess.free_reaches program shape oshape definitions linked library found
    backing pool i time state input result output buffer count kind mode stack matched bounded
    defined hk hm allowed executed writable separate resolves
...
theorem deriv_output_reaches (shape : Tensor.Shape) (definitions : CLoops.Calls.Definitions)
    ...
    ∃ finalHeap,
      ...
      Transition.Reaches (fun s t => CCalls.Events.internalNext program s = some t)
        ...
        (.returning (.integer 0)
          (written finalHeap buffer result shape.volume) stack) := by
  exact TensorDerivativeSuccess.output_reaches program shape definitions linked library found jacFound
    backing pool i time state input result J buffer count kind mode stack matched bounded bounded2
    defined hk hm allowed executed writable separate separateOutput adds resolves jacResolves
...
theorem null_deriv_behaviors (shape : Tensor.Shape) (hasOutput : Bool) (heap : Heap)
  ...
  apply GuardedCalls.null_behaviors program (derivFunction shape hasOutput)
    (Runtime.modeGuard .getDerivatives :: derivCountReject shape.volume ::
      (TensorDerivativePreflight.body ++
    ...
    (by simp [derivFunction, derivCheckedFunction, derivCheckedBody, Runtime.require, List.append_assoc])
...
theorem derivBody_printable (shape : Tensor.Shape) (hasOutput : Bool) :
  ...
  have dType : TypeSpelling RuntimePrinter.typedefs "double" := .named (.primitive (by decide +kernel))
  have flagType : TypeSpelling RuntimePrinter.typedefs "int" := .named (.primitive (by decide +kernel))
  have inputType : TypeSpelling RuntimePrinter.typedefs "const double *" :=
    .pointer (text := "const double") (.const (text := "double") dType)
  have iType : TypeSpelling RuntimePrinter.typedefs "Instance *" :=
    .pointer (text := "Instance") (.named (.typedefName (by decide +kernel) (by decide +kernel)))
  have fType : TypeSpelling RuntimePrinter.typedefs "fmi3Float64 *" :=
    .pointer (text := "fmi3Float64") (.named (.typedefName (by decide +kernel) (by decide +kernel)))
...
structure DerivContract (shape : Tensor.Shape) (hasOutput : Bool) (text : String) : Prop where
  ...
  numericalFailure : TensorDerivativeDiscard.Contract shape hasOutput
