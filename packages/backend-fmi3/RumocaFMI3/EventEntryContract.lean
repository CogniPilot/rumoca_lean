...
theorem message_collected (model : Solve.FMI3Model source) (entry : Entry) :
    ErrorCalls.rejectionMessage ∈ functionTexts (Runtime.function model (signature entry)) := by
  cases entry <;>
    simp [functionTexts, Runtime.function, body, Runtime.require, Runtime.instancePrefix,
      ...
      Runtime.call, Runtime.setMode, Runtime.put, Runtime.ok, ErrorCalls.rejectionMessage,
      statementTexts, expressionTexts, tail, Entry.command, Runtime.eventTime,
      Runtime.raiseField, Runtime.lt, Runtime.field, Runtime.v]
...
structure PreparedContract (model : Solve.FMI3Model source) (entry : Entry) (sigs : List Signature)
...
theorem prepared_correct (model : Solve.FMI3Model source) (entry : Entry) (sigs : List Signature)
  ...
  · intro E objects firstBlock
    ...
    apply quiet_correct objects (pool.addresses firstBlock) model entry program
    rw [actual]
  ...
  · intro before firstBlock signed objects heap frame
    ...
    obtain ⟨message, messageBound⟩ := LiteralPreparation.message_bound model sigs made (signature entry) member
      ErrorCalls.rejectionMessage (message_collected model entry) firstBlock
    ...
    · intro E program actual
      ...
      exact suppressed_correct objects literals model entry program message heap defined helper messageBound
    · intro program actual
      obtain ⟨defined, helper⟩ := definitions Invocation program actual
      exact logged_correct objects literals model entry program category message heap signed defined helper
...
structure FunctionContract (model : Solve.FMI3Model source) (entry : Entry) (sigs : List Signature) (text : String) : Prop where
  member : (signature entry) ∈ sigs
  printed : text = (Runtime.function model (signature entry)).render
  tokenization : Printer.FunctionTokenization RuntimePrinter.typedefs text (Runtime.function model (signature entry))
  prepared : ∀ pool, LiteralPreparation.prepare model sigs = some pool → PreparedContract model entry sigs pool

theorem rendered_contract (model : Solve.FMI3Model source) (entry : Entry) (sigs : List Signature)
    (unique : ((LiteralPreparation.functions model sigs).map (fun fn => fn.signature.name)).Nodup)
    (member : (signature entry) ∈ sigs) : FunctionContract model entry sigs (Runtime.function model (signature entry)).render := by
  refine ⟨member, rfl, ?_, fun _ made => prepared_correct model entry sigs unique member made⟩
