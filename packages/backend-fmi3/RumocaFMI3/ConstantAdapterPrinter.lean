...
theorem constantFunction_printable (model : Solve.FMI3Model source)
  ...
  split <;>
    first
      ...
      | exact (factory_printable model m.shape .me (TensorMetadata.constantToken m.name)).2
      | exact (factory_printable model m.shape .cs (TensorMetadata.constantToken m.name)).2
...
def FunctionsContract (model : Solve.FMI3Model source) (m : Solve.ConstantFMI3Model n)
    (signatures : List Signature) (adapter : String) : Prop :=
  ∃ text, adapter = (functionPrefix m.name ++ "#include \"model.c\"\n" ++
      ConstantFunctions.declarations m.shape (ConstantFunctions.rates m)) ++ text ∧
    FunctionsTokenization RuntimePrinter.typedefs text (ConstantFunctions.functions model m signatures)
...
theorem rendered_contract (model : Solve.FMI3Model source) (m : Solve.ConstantFMI3Model n)
  ...
  ⟨String.join ((ConstantFunctions.functions model m signatures).map Function.render),
