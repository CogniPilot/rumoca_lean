...
requests and derive Error, frames and ownership on rejected endpoints or counts, with all
...
theorem absent_empty_call (ty : AbsentVariables.VariableType) (write : Bool)
    ...
    (permitted : Reference.Allowed (AbsentVariables.accessCommand write) kind (state.phase.mode kind)) :
  ...
  have called := quiet.empty heap p references sizes values kind (state.phase.mode kind)
    stored.instanceStored.kind stored.instanceStored.mode_loaded permitted
...
theorem Result.absent_rejected (reason : AbsentVariables.Failure) (ty : AbsentVariables.VariableType) (write : Bool)
    ...
    Result objects retained owners p buffers heap after kind state (.absent (.reject reason ty write references sizes values n m)) := by
...
theorem absent_rejection_call (reason : AbsentVariables.Failure) (ty : AbsentVariables.VariableType) (write : Bool)
    (references sizes values : Option Address) (n m : UInt64)
    ...
    ∀ (heap : Heap) (p : Address) (buffers : Float64Buffers.Layout) (kind : Kind) (state : State)
      ...
      Stored heap p kind state → reason.Condition write kind (state.phase.mode kind) n m →
      ...
      CallContract model program objects retained owners p buffers heap kind state (.absent (.reject reason ty write references sizes values n m)) := by
  letI : CInterface := RuntimeEnvironment.interface header objects (pool.addresses firstBlock)
  ...
  have behavior (observed) :
      (Action.absent (.reject reason ty write references sizes values n m)).Behaves program heap p buffers observed ↔
  ...
  · have called := fun observed => (behavior observed).trans
      (quiet Invocation program actual reason p references sizes values n m kind (state.phase.mode kind) logger logging
    ...
    cases same
    exact ⟨⟨rfl, rfl⟩, Result.absent_rejected reason ty write references sizes values n m stored inPool ownership
      (fun _ _ => rfl) (.refl _)⟩
  · obtain ⟨called, _⟩ := logged program actual reason p logger references sizes values environment n m kind (state.phase.mode kind)
    ...
    · intro observed status after executed
      ...
      · have equal : observed = [⟨name, Logging.arguments environment category message⟩] ∧
        ...
        subst actualAfter
        exact ⟨⟨returnedStatus, rfl⟩, Result.absent_rejected reason ty write references sizes values n m stored inPool ownership
