...
/-! The shared counted tensor writer for arbitrary encoded binary64 results.
Clients prove expression evaluation in the current heap; typed stores and
counter/return execution remain generic. -/
noncomputable section
namespace Rumoca.CTensor.EncodedWriter
open CTree CMemory CMemory.TensorView CMemory.EncodedTensor
variable [interface : CInterface]

theorem write_step (expr : Expr) (values : Bits shape) (env : CBody.Locals) (types : CLoops.Types)
    ...
    (evaluated : CLoops.eval env types (EncodedTensor.written heap output values i.val) expr =
      some (.float64 values[i])) :
    CLoops.next (.running (.assign (indexed "out") expr :: rest) env types
      (EncodedTensor.written heap output values i.val)) =
      some (.running rest env types (EncodedTensor.written heap output values (i.val + 1))) := by
  have target := index_lvalue env (EncodedTensor.written heap output values i.val)
    "out" output i.val pointer counter
  have stored := EncodedTensor.store_next heap output values writable i
  simp only [indexed] at target ⊢
  change CBody.legacyExpressions.address env (EncodedTensor.written heap output values i.val)
...
theorem writer_reaches (expr : Expr) (values : Bits shape) (env : CBody.Locals) (types : CLoops.Types)
    ...
    (evaluated : ∀ i : Fin shape.volume,
      CLoops.eval (CLoops.counterEnv env "k" i.val) (CLoops.bindType types "k" .size)
        (EncodedTensor.written heap output values i.val) expr = some (.float64 values[i])) :
    Transition.Reaches CLoops.machine.step
      (.running (CLoops.counted "k" (.id "count") [.assign (indexed "out") expr] ++ [.ret none])
        env types heap)
      (.returned ⟨.void, EncodedTensor.written heap output values shape.volume⟩) := by
  have repeated := CLoops.loop_reaches "k" (.id "count") [.assign (indexed "out") expr] [.ret none]
    (fun _ => env) (CLoops.bindType types "k" .size) (EncodedTensor.written heap output values) shape.volume
...
end Rumoca.CTensor.EncodedWriter
