...
theorem diagonal_call_reaches (f : Syntax.Function) (valid : f.valid = true) (scope : DiagonalScope f)
  ...
  obtain ⟨finalHeap, ran, matrixReads, coefficientReads, frame⟩ :=
    emitDiagonal_correct (Arguments.locals f.parameters args) (Arguments.types f.parameters) locations
      definitions (library.setup definitions f valid args) diagonalDefined
      (Arguments.locals_absent f.parameters args _ scope.2) p plan layout output values coefficients heap
...
theorem diagonal_call_refines (f : Syntax.Function) (valid : f.valid = true) (scope : DiagonalScope f)
  ...
  obtain ⟨finalHeap, ran, matrixReads, coefficientReads, frame⟩ :=
    diagonal_call_reaches f valid scope p plan layout output matched definitions library diagonalDefined found
