...
theorem body_correct (f : Syntax.Function) (p : Program Γ shape) (plan : Plan p) (layout : Layout Γ)
    (matched : Syntax.Matches f p plan layout) : BodyCorrect f p plan layout := by
  ...
  rw [matched]
  exact emit_refines locals types locations definitions setup p plan layout values
...
theorem artifact_correct (source : String) (f : Syntax.Function)
    ...
    (matched : Syntax.Matches f p plan layout) : ArtifactContract source f p plan layout := by
  exact ⟨printed ▸ Syntax.render_denotes f valid, matched, body_correct f p plan layout matched⟩
