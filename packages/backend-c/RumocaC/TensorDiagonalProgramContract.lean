...
def DiagonalCallCorrectFor (f : Syntax.Function) (p : DiagonalProgram Γ shape)
    (plan : Plan p.coefficients) (layout : Layout Γ) (output : Buffer p.shape) : Prop :=
  ∀ [interface : CInterface] (definitions : CLoops.Calls.Definitions),
    LibraryFor (interface := interface) p.coefficients definitions → definitions Diagonal.function.signature.name = some Diagonal.function →
    definitions f.name = some f.tree → ∀ args, Arguments.Valid f.parameters args →
    ∀ (locations : Locations) (values : Env Binary64.Value Γ) (coefficients : Values shape) (heap : Heap),
    LayoutBound (Arguments.locals f.parameters args) locations layout → Represents locations layout heap values →
    Ready (Arguments.locals f.parameters args) locations p.coefficients plan layout heap →
...
def DiagonalArtifactContractFor (source : String) (f : Syntax.Function) (p : DiagonalProgram Γ shape)
    (plan : Plan p.coefficients) (layout : Layout Γ) (output : Buffer p.shape) : Prop :=
  Syntax.Denotes source f ∧ DiagonalScope f ∧ DiagonalMatches f p plan layout output ∧
    DiagonalCallCorrectFor f p plan layout output

theorem diagonal_artifact_correct_for (source : String) (f : Syntax.Function) (p : DiagonalProgram Γ shape)
    ...
    (matched : DiagonalMatches f p plan layout output) : DiagonalArtifactContractFor source f p plan layout output := by
  ...
  exact diagonal_call_refines_for f valid scope p plan layout output matched definitions library diagonalDefined found
    args arguments locations values coefficients heap bound represented ready reserved outputBound writable bounded executed

theorem DiagonalCallCorrectFor.to_full (h : DiagonalCallCorrectFor f p plan layout output) :
    DiagonalCallCorrect f p plan layout output := by
  intro interface definitions library
  exact h definitions (library.restrict p.coefficients)

theorem DiagonalArtifactContractFor.to_full (h : DiagonalArtifactContractFor source f p plan layout output) :
    DiagonalArtifactContract source f p plan layout output :=
  ⟨h.1, h.2.1, h.2.2.1, DiagonalCallCorrectFor.to_full h.2.2.2⟩

theorem diagonal_artifact_correct (source : String) (f : Syntax.Function) (p : DiagonalProgram Γ shape)
    (plan : Plan p.coefficients) (layout : Layout Γ) (output : Buffer p.shape)
    (printed : source = f.tree.render) (valid : f.valid = true) (scope : DiagonalScope f)
    (matched : DiagonalMatches f p plan layout output) : DiagonalArtifactContract source f p plan layout output :=
  DiagonalArtifactContractFor.to_full
    (diagonal_artifact_correct_for source f p plan layout output printed valid scope matched)
