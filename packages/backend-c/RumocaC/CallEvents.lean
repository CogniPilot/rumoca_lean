...
def resolve (p : Program E) (env : CBody.Locals) (heap : Heap) (callee : Expr) : Option String := do
  match ← Indirect.resolve env heap callee with
  | .named name => if name = "isfinite" then none else some name
  | .pointer address => p.addresses address

def enterCall (p : Program E) (s : CLoops.State) (resultType : String)
  ...
  | .running (stmt :: rest) env types heap => do
      ...
      let name ← resolve p env heap operand.callee
      let values ← arguments env heap operand.args
      return .calling name values heap (.caller operand.destination rest env types resultType stack)
  | .returned _ => none

def internalNext (p : Program E) : Typed.State → Option Typed.State :=
  Typed.nextWith (enterCall p) p.internal

inductive Step (p : Program E) : Typed.State → List E → Typed.State → Prop where
  | internal : internalNext p s = some t → Step p s [] t
...
def machine (p : Program E) : Transition.Events.Machine Typed.State E CBody.Result where
  step := Step p
...
theorem internal_preserves (p : Program E) (step : internalNext p s = some t) :
  ...
  CReadOnly.typed_nextWith (enterCall p) (fun _ _ _ _ next => enter_preserves p next) step

theorem step_preserves (step : Step p s events t) :
    CReadOnly.Preserves (CReadOnly.typedHeap s) (CReadOnly.typedHeap t) := by
  cases step with
  | internal step => exact internal_preserves _ step
...
theorem reaches_preserves (run : Transition.Events.Reaches (machine p).step s events t) :
  ...
  induction run with
  | refl => exact .refl _
  | next first rest ih => exact (step_preserves first).trans ih
...
theorem termination_preserves
  ...
  cases behavior with
  | terminates run done =>
      have preserved := reaches_preserves run
      rename_i last
...
theorem external_entry_exclusive (p : Program E) (found : p.externals name = some fn) :
    internalNext p (.calling name args heap stack) = none := by
  simp [internalNext, Typed.nextWith, p.disjoint name fn found]
...
theorem external_reaches (p : Program E) (found : p.externals name = some fn)
  ...
  simpa only [List.append_nil] using
    (Transition.Events.Reaches.next (Step.external found converted executed)
...
theorem body_step (p : Program E) (step : CLoops.next s = some t)
    ...
    internalNext p (.body s resultType stack) = some (.body t resultType stack) := by
  cases s with
  | returned => simp [CLoops.next] at step
  | running => simp [internalNext, Typed.nextWith, step]
...
theorem body_reaches (p : Program E) (run : Transition.Reaches CLoops.machine.step s t)
  ...
  induction run with
  | refl => exact .refl _
  | next first rest ih => exact .next (body_step p first resultType stack) ih
...
theorem internal_prefix (p : Program E)
    ...
    Transition.Events.Forced (machine p) s events result := by
  induction run with
  | refl => exact rest
  | next first tail ih => exact .next (Step.internal first) (internal_unique p first) (ih rest)
...
theorem tree_entry (p : Program E) (name : String) (args : List Value) (heap : Heap)
  ...
  simp [internalNext, Typed.nextWith, found, bound, typed]
...
theorem external_prefix (p : Program E) (found : p.externals name = some fn)
    ...
    Transition.Events.Forced (machine p) (.calling name args before stack) (events ++ tail) final := by
  refine .next (Step.external found converted executed) ?_ rest
...
theorem return_forced (p : Program E) (value : Value) (heap : Heap) :
    Transition.Events.Forced (machine p) (.returning value heap .done) [] ⟨value, heap⟩ := by
  refine Transition.Events.Forced.next (m := machine p) (first := []) (rest := [])
    (t := Typed.State.halted ⟨value, heap⟩) (Step.internal rfl) ?_ (.final rfl)
...
theorem external_behaviors (p : Program E) (found : p.externals name = some fn)
  ...
  simpa only [List.append_nil] using
    external_prefix p found converted executed unique (return_forced p result after)
