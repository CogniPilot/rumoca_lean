...
/-- Independent token grammar: two aliases and a structure with two fields.
...
theorem model_fields : fieldsNamed "Model" declarations =
    some [⟨"x", .real64⟩, ⟨"samplePeriod", .real64⟩] := rfl
...
structure Contract (source : String) : Prop where
  ...
  storage : fieldsNamed "Model" declarations = some [⟨"x", .real64⟩, ⟨"samplePeriod", .real64⟩]
